...
import { isTelegramMiniApp, shouldShowGoogle, shouldShowApple } from '../lib/platform-detection';

interface AuthScreenProps {
  onAuthSuccess: (user: AuthUser) => void;
}

type AuthMode = 'login' | 'register' | 'register-terms' | 'phone' | 'phone-verify';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onAuthSuccess }) => {
  ...
  const [confirmPassword, setConfirmPassword] = useState(''); // Password confirmation for registration
  ...
  // Safe platform detection (guards against SSR/initial load)
  const isRussian = typeof window !== 'undefined' 
    ? (window.Telegram?.WebApp?.initDataUnsafe?.user?.language_code === 'ru' || true)
    : true;

  // Platform detection for conditional OAuth rendering
  // These functions safely handle SSR/initial load by checking window first
  const showGoogle = typeof window !== 'undefined' ? shouldShowGoogle() : false;
  const showApple = typeof window !== 'undefined' ? shouldShowApple() : false;
  const isTelegram = typeof window !== 'undefined' ? isTelegramMiniApp() : false;
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'register') {
        ...
        if (!dateOfBirth) {
          setError(isRussian ? 'Пожалуйста, укажите дату рождения' : 'Please enter your date of birth');
          setIsLoading(false);
          return;
        }
        
        // Проверка возраста - блокировка регистрации если < 18
        const birthDate = new Date(dateOfBirth);
        const today = new Date();
        
        // Проверка валидности даты
        if (birthDate > today) {
          setError(isRussian ? 'Дата рождения не может быть в будущем' : 'Date of birth cannot be in the future');
          setIsLoading(false);
          return;
        }
        ...
        if (actualAge < 18) {
          setError(isRussian ? 'Доступ разрешен только пользователям в возрасте 18 лет и старше' : 'Access is allowed only for users aged 18 and above');
          setIsLoading(false);
          return;
        }
        ...
        if (password !== confirmPassword) {
          setError(isRussian ? 'Пароли не совпадают' : 'Passwords do not match');
          setIsLoading(false);
          return;
        }
        
        // Save registration data and proceed to terms step
        console.log('✅ Step 1 validated, proceeding to terms step...');
        setRegistrationData({
          email,
          password,
          fullName,
        ...
        setMode('register-terms');
        setIsLoading(false);
        return;
      } else if (mode === 'register-terms') {
        // Step 2: Complete registration with terms acceptance
        if (!registrationData) {
          setError(isRussian ? 'Ошибка: данные регистрации не найдены' : 'Error: registration data not found');
          setIsLoading(false);
          return;
        }
        
        console.log('🔄 Step 2: Completing registration with terms acceptance...');
  ...
  return (
    <div 
      className="min-h-screen h-screen bg-gradient-to-br from-black via-purple-950 to-indigo-950 flex items-center justify-center p-4"
      style={{
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="flex gap-2 mb-6">
            ...
            <button
              onClick={() => {
                ...
                setConfirmPassword(''); // Reset password confirmation when switching to register
              }}
              className={`flex-1 py-2 px-4 rounded-xl text-sm font-medium transition-all ${
                mode === 'register' || mode === 'register-terms'
                  ? 'bg-purple-500/20 text-white border border-purple-400/30'
                  : 'text-white/60 hover:text-white hover:bg-white/5'
              }`}
            >
          ...
          {(mode === 'login' || mode === 'register') && (
            <form onSubmit={handleEmailAuth} className="space-y-4">
              ...
              {mode === 'register' && (
                <div>
                  <label className="block text-white/70 text-sm mb-2">
                    {isRussian ? 'Подтвердите пароль' : 'Confirm Password'}
                  </label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-white/40" size={18} />
                    <input
                  ...
                  {confirmPassword && password !== confirmPassword && (
                    <p className="text-red-400 text-xs mt-1">
                      {isRussian ? 'Пароли не совпадают' : 'Passwords do not match'}
                    </p>
                  )}
                </div>
              )}
              ...
              {mode === 'register' && (
                <>
                  ...
                  <div>
                    <label className="block text-white/70 text-sm mb-2">
                      {isRussian ? 'Дата рождения' : 'Date of Birth'}
                    </label>
                    <input
                      type="date"
                      value={dateOfBirth}
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-purple-400/50 transition-colors"
                      required={mode === 'register'}
                    />
                  </div>
                  ...
                  <p className="text-white/50 text-xs mt-1">
                    {isRussian 
                      ? 'Доступ разрешен только пользователям 18+' 
                      : 'Access allowed only for users 18+'}
                  </p>
                </>
              )}
              ...
              {mode === 'register' && (
                <button
                  type="submit"
                  disabled={isLoading || !gender || !dateOfBirth || !confirmPassword || password !== confirmPassword}
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      <span>{isRussian ? 'Продолжить' : 'Continue'}</span>
                      <ArrowRight size={18} />
                    </>
                  )}
                </button>
              ...
              {mode === 'login' && (
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      <span>{isRussian ? 'Войти' : 'Sign In'}</span>
                      <ArrowRight size={18} />
                    </>
                  )}
                </button>
            ...
            <form onSubmit={handlePhoneAuth} className="space-y-4">
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    <Phone size={18} />
                    <span>{isRussian ? 'Войти по номеру телефона' : 'Sign in with phone number'}</span>
                    <ArrowRight size={18} />
                  </>
                )}
              </button>
          ...
          {/* OAuth кнопки (Google, Apple) - только для Email/Password форм, с учетом платформы */}
          {(mode === 'login' || mode === 'register') && (showGoogle || showApple) && (
            <div className="mt-6 pt-6 border-t border-white/10 space-y-3">
              <p className="text-white/60 text-xs text-center mb-3">
                {isRussian ? 'Или войдите через' : 'Or sign in with'}
              </p>
              
              {/* Google OAuth - только для Web и Android */}
              {showGoogle && (
                <button
                  onClick={async () => {
                    setIsLoading(true);
                  ...
                  disabled={isLoading}
                  className="w-full bg-white/10 hover:bg-white/20 border border-purple-400/30 py-3 rounded-xl text-white font-medium flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg backdrop-blur-sm"
                >
              ...
              {/* Apple OAuth - только для Web и iOS */}
              {showApple && (
                <button
                  onClick={async () => {
                    setIsLoading(true);
            ...
            onAccept={async () => {
              ...
              try {
                ...
                if (result.success && result.user) {
                  ...
                  setError(result.error || (isRussian ? 'Ошибка регистрации' : 'Registration error'));
                  if (window.Telegram?.WebApp?.showAlert) {
                    WebApp.showAlert(result.error || (isRussian ? 'Ошибка регистрации' : 'Registration error'));
                  }
                }
              } catch (err) {
                const errorMsg = err instanceof Error ? err.message : 'Unknown error';
        ...
        {/* Telegram auto-login happens silently via useEffect, no UI text needed for compliance */}
      </motion.div>
    </div>
  );
};
